...
public class DiffTest {
	...
	List<String> lines = IOUtils.readLines(FileUtils
		.openInputStream(new File("httpd.conf.diff")));
	...
	for (Delta delta : diff.getDeltas()) {
		...
		if (tag == Tag.INSERT) {
		    ...
		    throw new IllegalStateException("Unknown pattern tag: "
			    + tag);
		}
	...
	for (SideBySideView.Line line : view.lines) {
	    System.out
		    .printf("<left class=\"%s\"><num>%s</num><text>%s</text></left>"
			    + "<right class=\"%s\"><num>%s</num><text>%s</text></right>\n",
			    line.left.cssClass, line.left.getNumber(),
			    line.left.text, line.right.cssClass,
			    line.right.getNumber(), line.right.text);
	}
    }
